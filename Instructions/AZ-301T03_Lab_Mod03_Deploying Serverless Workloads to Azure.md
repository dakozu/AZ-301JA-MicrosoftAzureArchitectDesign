...
2. **New** ブレードの上部の **マーケットプレースの検索** テキスト ボックスで、**Template Deployment** と入力し、**Enter** を押します。

3. **Everything** ブレードの、検索結果で **テンプレートのデプロイメント** をクリックします。

4. **テンプレートのデプロイ** ブレードで、 **作成** ボタンをクリックします。

5. **カスタム デプロイ** ブレードで、**エディターで独自のテンプレートを作成** リンクをクリックします。
...
10. **Custom deployment** ブレードに戻り、次のタスクを実行します:
    ...
    - **App Name** テキスト ボックスに、新しい Function App のユニークな名前を入力します。

    - **Sku** ドロップダウン リストで、**B1** オプションを選択します。

    - **Worker Size** ドロップダウン リストは既定値に設定したままにします。

    - **Storage Account Type** ドロップダウン リストは、既定値に設定したままにします。
...
2. **Resource groups** ブレードで、 **AADesignLab0503-RG** をクリックします。

3. **AADesignLab0503-RG** ブレードで、前のタスクで作成した Function App を表すエントリをクリックします。

4. Function App ブレードで **Url** エントリを検索し、ハイパーリンクをクリックして、新しいブラウザ タブで Function App のランディング ページが表示します。
